...
# matplotlib is only needed for the cosmetic PNG plots attached to benchmark
# results. It may be missing (or unusable in headless containers) on some
# delivery environments; a benchmark's pass/fail must never depend on it,
# so degrade gracefully to "no plots" in that case.
try:
    import matplotlib
...
def plot_latency_and_speedup(results: Iterable, output_dir: str, op_name: str):
    if plt is None:
        print(f"[visualization] matplotlib unavailable, skip plots for {op_name}")
        return
    ...
    for (dtype, mode), items in grouped.items():
        try:
            ...
            print(f"[visualization] skip plots for {op_name} ({dtype}, {mode}): {e}")
            continue
        ...
        try:
            ...
            # Plotting is cosmetic — never let it fail a benchmark.
            print(f"[visualization] plot failed for {op_name} ({dtype}, {mode}): {e}")
            plt.close("all")
