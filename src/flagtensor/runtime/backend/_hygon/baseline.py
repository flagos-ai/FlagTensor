...
"""Hygon-native baseline classes for FlagTensor benchmarks.

On the Hygon DCU (Deep Computing Unit, CUDA-compatible via ``torch_hcu``):
...
* **PyTorch native ops DO use vendor-optimised kernels.**  ``torch.matmul``
  ...
  dispatch through the ``torch_hcu`` symbol-rewrite hook to Hygon's DCU
  vendor libraries (``libdtkblas.so``, ``libdtkdnn.so``, ...) — the same
  position cuTensor occupies on NVIDIA.

* **This mirrors the FlagGems convention** and the PPU / Iluvatar /
  Kunlunxin backends: for platforms without a cuTensor-equivalent vendor
  library, ``torch.*`` ops are the native baseline.  This is NOT a
  "fallback" — it is the vendor-supplied optimised path on Hygon DCU.
...
Hygon, exactly as it does on PPU / Iluvatar / Kunlunxin.
"""
...
def elementwise_trinary(
    ...
    """Hygon-native elementwise trinary function (mirror of cutensor.elementwise_trinary)."""
...
# same resolution logic works on Hygon without modifying benchmark_core.
