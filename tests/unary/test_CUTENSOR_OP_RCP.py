...
def test_rcp_correctness(shape, dtype):
    ...
    x = torch.where(torch.randn(shape, device=_device_str, dtype=dtype) >= 0, torch.randn(shape, device=_device_str, dtype=dtype) + 1e-3, torch.randn(shape, device=_device_str, dtype=dtype) - 1e-3)
