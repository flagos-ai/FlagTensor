...
from flagtensor.config import DEFAULT_BENCHMARK_DTYPES, DEFAULT_LOG_BENCHMARK_SHAPES
from flagtensor.cutensor import CUTENSOR_AVAILABLE, CuTensorLog
from flagtensor.visualization import plot_latency_and_speedup, write_benchmark_csv

OP_NAME = "CUTENSOR_OP_LOG"
...
class LogBenchmark(Benchmark):
    def __init__(self):
        super().__init__(
            ...
            config=BenchmarkConfig(
                ...
                shapes=tuple(DEFAULT_LOG_BENCHMARK_SHAPES),
            ),
        )
        self.baselines = {}
    ...
    def get_input_iter(self, dtype: torch.dtype):
        for shape in self.config.shapes:
            yield (torch.empty(shape, device=self.device, dtype=dtype).uniform_(1e-3, 8.0),)

    def baseline_impl(self, x):
        baseline = self.baselines.get(x.dtype)
        if baseline is None:
            baseline = CuTensorLog(dtype=x.dtype)
