...
from flagtensor.config import DEFAULT_BENCHMARK_DTYPES, DEFAULT_ASIN_BENCHMARK_SHAPES
...
try:
    from flagtensor.cutensor import CuTensorAsin as _BaselineClass
except ImportError:
    _BaselineClass = None
try:
...
class AsinBenchmark(Benchmark):
    ...
    def get_input_iter(self, dtype: torch.dtype):
        for shape in self.config.shapes:
            yield (torch.empty(shape, device=self.device, dtype=dtype).uniform_(-1, 1),)
