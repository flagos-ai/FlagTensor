...
class GettBenchmark(Benchmark):
    ...
    def get_input_iter(self, dtype: torch.dtype):
        for shape_a, shape_b in self.config.shapes:
            _, _, _, c_shape, _ = _gett_case(shape_a, shape_b)
            yield (
    ...
    def baseline_impl(self, a, b, c):
        ...
        mode_a, mode_b, mode_d, _, _ = _gett_case(tuple(a.shape), tuple(b.shape))
    ...
    def triton_impl(self, a, b, c):
        mode_a, mode_b, mode_d, _, _ = _gett_case(tuple(a.shape), tuple(b.shape))
        return gett(a, b, c=c, alpha=1.25, beta=0.5, mode_a=mode_a, mode_b=mode_b, mode_c=mode_d, mode_d=mode_d)

    def reference_impl(self, a, b, c):
        _, _, _, _, reference = _gett_case(tuple(a.shape), tuple(b.shape))
        return _gett_reference(a, b, c, reference)
