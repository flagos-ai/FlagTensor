...
from flagtensor.config import DEFAULT_BENCHMARK_DTYPES, DEFAULT_TTGT_BENCHMARK_SHAPES
from flagtensor.cutensor import CUTENSOR_AVAILABLE, CuTensorContraction, ttgt as cutensor_ttgt
from flagtensor.ops.CUTENSOR_OP_GETT import _launch_gett_like_kernel
from flagtensor.ops.CUTENSOR_OP_TTGT import _is_default_2d_ttgt_case
from flagtensor.visualization import plot_latency_and_speedup, write_benchmark_csv

OP_NAME = "CUTENSOR_OP_TTGT"
...
def _ttgt_case(shape_a, shape_b):
    ...
    raise ValueError("unsupported TTGT benchmark shape combination")
...
class TtgtBenchmark(Benchmark):
    ...
    def get_input_iter(self, dtype: torch.dtype):
        for shape_a, shape_b in self.config.shapes:
            _, _, _, c_shape, _ = _ttgt_case(shape_a, shape_b)
            yield (
    ...
    def baseline_impl(self, a, b, c):
        mode_a, mode_b, mode_d, _, _ = _ttgt_case(tuple(a.shape), tuple(b.shape))
        return cutensor_ttgt(a, b, c=c, alpha=1.25, beta=0.5, mode_a=mode_a, mode_b=mode_b, mode_c=mode_d, mode_d=mode_d)

    def triton_impl(self, a, b, c):
        mode_a, mode_b, mode_d, _, _ = _ttgt_case(tuple(a.shape), tuple(b.shape))
        return ttgt(a, b, c=c, alpha=1.25, beta=0.5, mode_a=mode_a, mode_b=mode_b, mode_c=mode_d, mode_d=mode_d)

    def reference_impl(self, a, b, c):
        _, _, _, _, reference = _ttgt_case(tuple(a.shape), tuple(b.shape))
        return _ttgt_reference(a, b, c, reference)

    def build_triton_kernel_callable(self, *args):
        a, b, c = args
        mode_a, mode_b, mode_d, _, _ = _ttgt_case(tuple(a.shape), tuple(b.shape))
        if not _is_default_2d_ttgt_case(a, b, c, mode_a, mode_b, mode_d, mode_d):
            return None
        out = torch.empty((a.shape[1], b.shape[0]), device=a.device, dtype=a.dtype)

        def run_kernel():
            return _launch_gett_like_kernel(a, b, c, out, 1.25, 0.5, trans_a=True, trans_b=True)
    ...
    def build_baseline_kernel_callable(self, *args):
        ...
        def run_kernel():
            return baseline(a_t, b_t, c=c, alpha=1.25, beta=0.5, mode_a=mode_a, mode_b=mode_b, mode_c=mode_d, mode_d=mode_d)
