...
from flagtensor.config import DEFAULT_BENCHMARK_DTYPES, DEFAULT_RCP_BENCHMARK_SHAPES
...
try:
    from flagtensor.cutensor import CuTensorRcp as _BaselineClass
except ImportError:
    _BaselineClass = None
try:
...
class RcpBenchmark(Benchmark):
    ...
    def get_input_iter(self, dtype: torch.dtype):
        for shape in self.config.shapes:
            yield (make_nonzero_tensor(shape, self.device, dtype),)
