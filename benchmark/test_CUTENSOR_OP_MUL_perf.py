...
from flagtensor.config import DEFAULT_BENCHMARK_DTYPES, DEFAULT_MUL_BENCHMARK_SHAPES
...
try:
    from flagtensor.cutensor import CuTensorMul as _BaselineClass
except ImportError:
    _BaselineClass = None
try:
...
class MulBenchmark(Benchmark):
    ...
    def get_input_iter(self, dtype: torch.dtype):
        for shape in self.config.shapes:
            yield (
                torch.empty(shape, device=self.device, dtype=dtype).uniform_(-8.0, 8.0),
                torch.empty(shape, device=self.device, dtype=dtype).uniform_(-8.0, 8.0),
            )
