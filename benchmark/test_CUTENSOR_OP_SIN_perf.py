...
from flagtensor.config import DEFAULT_BENCHMARK_DTYPES, DEFAULT_SIN_BENCHMARK_SHAPES
from flagtensor.cutensor import CUTENSOR_AVAILABLE, CuTensorSin
from flagtensor.visualization import plot_latency_and_speedup, write_benchmark_csv

OP_NAME = "CUTENSOR_OP_SIN"
...
class SinBenchmark(Benchmark):
    def __init__(self):
        super().__init__(
            ...
            config=BenchmarkConfig(
                ...
                shapes=tuple(DEFAULT_SIN_BENCHMARK_SHAPES),
            ),
        )
        self.baselines = {}
    ...
    def baseline_impl(self, x):
        ...
        if baseline is None:
            baseline = CuTensorSin(dtype=x.dtype)
