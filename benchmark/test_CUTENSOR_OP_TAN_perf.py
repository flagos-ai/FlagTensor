...
from flagtensor.config import DEFAULT_BENCHMARK_DTYPES, DEFAULT_TAN_BENCHMARK_SHAPES
from flagtensor.cutensor import CUTENSOR_AVAILABLE, CuTensorTan
from flagtensor.visualization import plot_latency_and_speedup, write_benchmark_csv

OP_NAME = "CUTENSOR_OP_TAN"
RESULTS_ROOT = os.path.join(os.path.dirname(__file__), "results")
RESULTS_DIR = os.path.join(RESULTS_ROOT, OP_NAME)
CSV_PATH = os.path.join(RESULTS_DIR, "benchmark.csv")


class TanBenchmark(Benchmark):
    def __init__(self):
        super().__init__(
            op_name=OP_NAME,
            config=BenchmarkConfig(
                dtypes=tuple(DEFAULT_BENCHMARK_DTYPES),
                shapes=tuple(DEFAULT_TAN_BENCHMARK_SHAPES),
            ),
        )
        self.baselines = {}
    ...
    def baseline_impl(self, x):
        ...
        if baseline is None:
            baseline = CuTensorTan(dtype=x.dtype)
